        ...
        <div className="relative animate-slide-in-delay pt-12">
          ...
          {filteredModels?.map((model, index) => (
            ...
            >
              ...
              >
                {/* Timeline dot */}
                <div className="absolute left-6 h-4 w-4 rounded-full border-2 border-blue-500 bg-mf-milk-300 sm:left-1/2 sm:-translate-x-1/2" />

                {/* Content card */}
                <Link
                  href={`/models/${encodeURIComponent(model.name ?? "")}`}
                  className="block rounded-lg border border-[#e0e0e0] bg-mf-milk-300 p-3 shadow transition-shadow hover:shadow-md sm:p-4"
                >
